...
def create_bus_scheduling_model():
    ...
    solver = pywraplp.Solver.CreateSolver('GLOP')  # You can choose a different solver
    ...
    # Objective Function
    solver.Minimize(sum(x[i, j] + y[i, j] for i in range(1, num_routes + 1) for j in range(1, num_shifts + 1)))
    ...
    # Constraint 5
    solver.Add(solver.Sum(x[i, j] + y[i, j] for i in range(1, num_routes + 1) for j in range(1, num_shifts + 1))
               <= 93 * sum(w))
...
if status == pywraplp.Solver.OPTIMAL:
    ...
    for i in range(1, num_routes + 1):
        for j in range(1, num_shifts + 1):
            ...
            print(f"  Bus Type-I Trips (x): {x[i, j].solution_value()}")
            print(f"  Bus Type-II Trips (y): {y[i, j].solution_value()}")
else:
